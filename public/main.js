document.addEventListener('DOMContentLoaded', function () {
    ...
    function filtrarDados(dados, periodo, valor, area) {
      ...
      const filtroPeriodo = {
        'all': () => true,
        'last-30': item => new Date(item.data) >= new Date(agora.setDate(agora.getDate() - 30)),
        'last-90': item => new Date(item.data) >= new Date(agora.setDate(agora.getDate() - 90)),
        'last-year': item => new Date(item.data) >= new Date(agora.setFullYear(agora.getFullYear() - 1)),
      };
